 ...
 switch (mode)
   {
    ...
    case 25: if((timerElst[0]+timerElst[2])==0) timerElst[1]=buf&1;                 // П9 реверсный режим реле / симистора (нагрева / охлаждения)
             else timerElst[1]=buf;                                                 // П9 время работы электростатики
        break;
    case 26: timerElst[2]=buf; break;                                               // П10-время паузы электростатики    
    case 27: Priority = buf&7; break;                                               // П11 приоритет датчиков  
    case 28: if((timerElst[0]+timerElst[2])==0){if(buf<1) buf=1; ocra1 = (unsigned int)buf;}        // П12 Пропорциональный кофф.
             else {if(buf<20) buf=20; ocra1 = (unsigned int)62500/(buf&0x1FF); OCR1A =ocra1;}       // П12 частота импульсов
        break;             
    case 29: if((timerElst[0]+timerElst[2])==0){if(buf<100)  buf=100;  ocrb1 = (unsigned int)buf;}  // П13 Интегральный кофф.
             else {if(buf<5)  buf=5;  ocrb1 = (unsigned int)ocra1*(buf&0x3f)/100; OCR1B =ocrb1;}    // П13 скважность
        break;      
