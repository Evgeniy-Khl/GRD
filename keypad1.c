              ...
              switch (mode)
                {
                 ...
                 case 6: buf=0; break;                // У04 (сек.) Задержка включения электростатики.
              ...
              switch (mode)
                {
                 ...
                 case 20: buf=timerHum[0]; break;    // П04 Время отключения увлажнителя 1ед. = 0,25 сек. (40ед. = 10 сек.)
                 case 21: buf=timerHum[1]; break;    // П05 Время включения увлажнителя 1ед. = 0,25 сек. (4ед. = 1 сек.)
                 ...
                 case 28: buf=humidMin; break;       // П12 температура с которой начинается увлажнение
                 case 29: buf=coolMax; break;        // П13 температура с которой ЗАПРЕЩЕНО включение охлаждения
...
void saveset(void)
{
 switch (mode)
   ...
   // case 6: timerMain[0]=buf; break;
    ...
    case 17: if(buf>maxSP/4) buf=maxSP/4; else if(buf<0) buf=0; OvHeat = buf; break;// П01-максимально допустимое отклонение температуры 
    case 18: if(buf>maxSP/8) buf=maxSP/8; else if(buf<0) buf=0; Hyst = buf; break;  // П02-гистерезис терморегулятора
    case 19: if(buf>2) buf=2; else if(buf<0) buf=0; DimTmr = buf; break;            // П03-Размерность таймера
    case 20: timerHum[0] = buf; break;                                              // П04 Время отключения увлажнителя 1ед. = 0,25 сек. (40ед. = 10 сек.)
    case 21: timerHum[1] = buf; break;                                              // П05 Время включения увлажнителя 1ед. = 0,25 сек. (4ед. = 1 сек.)
    case 22: if(buf>99) buf=99; else if(buf<0) buf=0; timeCool[Step] = buf; break;  // П06-Время продувки в минутах.
    case 23: if(buf>130) buf=130; else if(buf<0) buf=0; maxSP = buf; break;         // П07-Ограничение максимальной температуры.
    ...
    case 28: humidMin = buf&0x7F; break;                                            // П12 температура с которой начинается увлажнение
    case 29: coolMax = buf&0x3F; break;                                             // П13 температура с которой ЗАПРЕЩЕНО включение охлаждения
